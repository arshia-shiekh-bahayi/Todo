...
class ActivationApiView(APIView):
    ...
    def get(self, request, token, *args, **kwargs):
        ...
        try:
            ...
            return Response(
                {"details": "token not valid"}, status=status.HTTP_400_BAD_REQUEST
            )
        user_obj = User.objects.get(pk=user_id)
        if user_obj.is_verified:
            return Response(
                {"details": "Your accounts had been verified already"},
        ...
        return Response(
            {"details": "Your accounts have been verified successfully"},
            status=status.HTTP_200_OK,
        )
